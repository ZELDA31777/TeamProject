		...
		<ul class="nav nav-tabs nav-fill" id="myTab" role="tablist">
			<li class="nav-item" role="presentation">
				<button class="nav-link active" id="profile-tab"
					...
					role="tab" aria-controls="profile" aria-selected="true">공지</button>
			</li>
			<li class="nav-item" role="presentation">
						...
						<div class="col p-4 d-flex flex-column position-static">
									...
									<div class="row">
										<div class="col-md-3">
											...
											<img src="/upload/${pvo.thumbnail }"
												...
												onclick="pSend(this);" data-pno="${pvo.pno}">
										</div>
										<div class="col-md-2 ">
											<!-- 알람 대상의 정보 -->
											<div class="fzGgeZ">
												...
												<p class="eswqdM">
													보증금/${pvo.deposit }만원<br>월&nbsp;세/${pvo.rent}만원
												</p>
											</div>
										</div>
										<!-- 알람 대상의 상세 정보 -->
										<div class="col-md-6 bSKZAI">
											<p class="eHtuHa">
												...
												, ${Math.round(pvo.area * 100) / 100.0}m², 관리비 ${pvo.manage }만
											</p>
							...
							<c:if test="${empty alertList}">
								<div class="container-fluid text-center"
									style="margin-top: 5%; margin-bottom: 5%;">관심지역에 새로 등록된
									매물이 없습니다.</div>
							</c:if>
						</div>
					</div>
						...
						<div class="col p-4 d-flex flex-column position-static">
							...
							<c:if test="${empty prevo }">
								...
								<div class="row">
									...
									<button type="button" id="iamportPayment01"
										class="btn btn-light">가입하기</button>
								</div>
							</c:if>
							...
							<c:if test="${!empty prevo }">
								...
								<div class="row">
									<p>프리미엄 멤버십 잔여일이</p>
									<p>
										<span>${dayCheck }</span>&nbsp;일 남았습니다.
									</p>
									<button type="button" id="iamportPayment02"
										class="btn btn-light">연장하기</button>
								</div>
							</c:if>
...
<form name="payform" action="/alert/premium_insert_pro.do" method="post">
	<input type="hidden" name="${_csrf.parameterName }"
		value="${_csrf.token }"> <input type="hidden" name="username"
		value="${username}"> <input type="hidden" id="merchant_uid1"
		name="merchant_uid">
</form>

<form name="payUpdateform" action="/alert/premium_update_pro.do"
	method="post">
	<input type="hidden" name="${_csrf.parameterName }"
		value="${_csrf.token }"> <input type="hidden" name="username"
		value="${username}"> <input type="hidden" id="merchant_uid2"
		name="merchant_uid">
</form>
