...
<main class="container">
		...
		<div class="col-md-6 align-self-center" style="position: relative;">
			...
			<div class="row mb-2">
				<div class="col-md-6 py-2 px-2">
					<div class="img_wrap">
						<img src="/upload/${picList[0]}" style="width: 100%">
					</div>
				</div>
				<div class="col-md-6 py-2 px-2">
					<div class="img_wrap">
						<img src="/upload/${picList[1]}" style="width: 100%">
					</div>
				</div>
				<div class="col-md-6 py-2 px-2">
					<div class="img_wrap">
						<img src="/upload/${picList[2]}" style="width: 100%">
					</div>
				</div>
				<div class="col-md-6 py-2 px-2">
					<div class="img_wrap">
						<img src="/upload/${picList[3]}" style="width: 100%">
					</div>
				</div>
			</div>
		</div>
	...
	<div class="row g-5">
		<div class="col-md-8">
			...
			<form name="pr_modify" action="/product/product_modify_pro.do"
				method="post" enctype="multipart/form-data">
				<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"> 
				<input type="hidden" name="addr" id="addr" value="test"> 
				<input type="hidden" name="pno" value="${pvo.pno }"> 
				<input type="hidden" name="lat" id="lat" value="${pvo.lat }"> 
				<input type="hidden" name="lng" id="lng" value="${pvo.lng }">
				<input type="hidden" name="thumbnail" value="${pvo.thumbnail }">
				<input type="hidden" name="picture" value="${pvo.picture }">
					...
					<li>
						<div class="row"
							...
							<div class="gEjpOz col-md-9">
								<input type="file" class="form-control w-75" id="thumbnailFile"
									name="thumbnailFile" accept="image/*"
									onchange="selectFile(this);" data-idx="0">
						...
						<div class="row"
							...
							<div class="gEjpOz col-md-9" id="pictureDiv">
								...
								<div class="position-relative">
									<input type="file" class="form-control w-75 d-inline-block"
										name="pictureFile" accept="image/*"
										onchange="selectFile(this)" data-idx="1">
									<button
										class="btn btn-outline-success position-absolute end-0 me-3"
										id="picturePlus" type="button">
								...
								<div class="position-relative mt-3">
									<input type="file" class="form-control w-75 d-inline-block"
										name="pictureFile" accept="image/*"
										onchange="selectFile(this)" data-idx="2">
								</div>
								<div class="position-relative mt-3">
									<input type="file" class="form-control w-75 d-inline-block"
										name="pictureFile" accept="image/*"
										onchange="selectFile(this)" data-idx="3">
								</div>
								<div class="position-relative mt-3">
									<input type="file" class="form-control w-75 d-inline-block"
										name="pictureFile" accept="image/*"
										onchange="selectFile(this)" data-idx="4">
								</div>
							</div>
						</div>
					</li>
						...
						<div class="row"
							...
							<div class="gEjpOz col-md-9">
								<input name="rent" placeholder="월세" id="rent"
									class="styled__PriceInput-sc-1xx7qac-2 ceburN"
									value="${pvo.rent }">
								<p class="styled__Won-sc-1xx7qac-4 ceMnZE">
							...
							<div class="gEjpOz col-md-9">
								<div>
									<input name="manage" placeholder="관리비" id="manage"
										class="ceburN" value="${pvo.manage }"> <label><input
							...
							<div class="gEjpOz col-md-9">
								...
								<div class="eVAGdp">
									<div class="bDaqny">
										...
										<div class="row gy-3">
											<div class="col-4">
												<input type="text" class="kHDHPO w-100" name="addr1"
													id="addr1" value="${pvo.addr1 }" readonly>
											</div>
											<div class="col-4">
												<input type="text" class="kHDHPO w-100" name="addr2"
													id="addr2" value="${pvo.addr2 }" readonly>
											</div>
											<div class="col-4">
											...
											<div class="col-12">
												<input type="text" class="form-control kHDHPO" name="addr3"
													id="addr3" value="${pvo.addr3 }" required>
									...
									<td class="kYmWlH">
										<ul class="gMKyVw" style="list-style: none;">
											<li><label class="blvQKJ"><input type="radio"
													id="roomType_1" value="0" name="type">
													<p>원룸</p></label></li>
											<li><label class="blvQKJ"><input type="radio"
													id="roomType_2" value="1" name="type">
													<p>오피스텔</p></label></li>
										</ul>
									</td>
								</div>
							...
							<div class="row"
								...
								<div class="gEjpOz flex col-md-9">
									<input class="ceburN" type="text" id="cal1"
										...
										name="area" type="text" id="cal2" value="${pvo.area}"
										onkeyup="calculator(2);">m2
								</div>
							</div>
						</li>
						<li>
				...
				<div class="fCMewi">
					...
					<div class="iOHiXt" style="list-style: none;">
						<div class="jGAWeI">
							<textarea name="contents" style="width: -webkit-fill-available;">${pvo.contents }</textarea>
						</div>
					</div>
				</div>
				...
				<div class="jtcdhj">
					<div class="styled__PriceWrap-cvrpi1-5 fzGgeZ">
						<div class="row">
							...
							<div class="col-md-2">
								<p class="eswqdM" id="deposit2">${pvo.deposit }</p>
							</div>
							<div class="col-md-1">
								<p class="eswqdM">/</p>
							</div>
							<div class="col-md-2">
								<p class="eswqdM" id="rent2">${pvo.rent }</p>
							</div>
						</div>
					</div>
					<ul class="bIRVVc">
						<li><div class="fYSHvV left">
								...
								<span id="roomType2"><c:if test="${pvo.type eq 1}">오피스텔</c:if>
							...
							<div class="styled__Item-cvrpi1-9 fYSHvV">
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none"
									...
									<path fill-rule="evenodd" clip-rule="evenodd"
										...
										fill="#222222"></path></svg>
								<span id="area2">${pvo.area }</span><span>㎡</span>
							</div></li>
						<li><div class="styled__Item-cvrpi1-9 fYSHvV left">
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none"
									...
									<path fill-rule="evenodd" clip-rule="evenodd"
										...
										fill="#222222"></path></svg>
								<span id="floor2">${pvo.floor }층</span> <span>/</span> <span
									id="totalfloor2">${pvo.totalfloor }층</span>
							</div>
							<div class="styled__Item-cvrpi1-9 fYSHvV">
								<svg width="24" height="24" viewBox="0 0 24 24" fill="none"
									xmlns="http://www.w3.org/2000/svg" style="flex: 0 0 auto;">
									<path fill-rule="evenodd" clip-rule="evenodd"
										...
										fill="#222222"></path></svg>
								<span id="manage2"><c:if test="${pvo.manage eq 0}">없음</c:if>
								<c:if test="${pvo.manage ne 0}">${pvo.manage }만원</c:if>
								 </span>
							</div></li>
						<li class="kRCkkj"><div>
								<p class="title">방/욕실</p>
								<p class="content">
									<span id="room2">${pvo.room }</span><span>개</span> / <span
										id="shower2">${pvo.shower }</span>개
								</p>
							</div>
							<div>
								<p class="title">위치</p>
								<p class="content">
									<span id="addrprint1">${pvo.addr1 }</span> <span
										id="addrprint2">${pvo.addr2 }</span> <span id="addrprint3">${pvo.addr3 }</span>
								</p>
							</div></li>
					</ul>
					...
					<div class="brpHbd">
						<button type="button" color="blue" class="fiEZwu"
							onClick="modify_()">
...
<script>
	...
	$(document).ready(function(){
		if(${pvo.type} =="0"){
			$(":radio[name='type'][value='0']").attr('checked', true);
		} else if (${pvo.type} =="1"){
			$(":radio[name='type'][value='1']").attr('checked', true);
		}
		document.getElementById('cal1').value = parseFloat(${pvo.area}) / 3.3058;

		$("#floor").val("${pvo.floor}").prop("selected", true);
		$("#totalfloor").val("${pvo.totalfloor}").prop("selected", true);
		
		if(${pvo.room} =="0"){
			$(":radio[name='room'][value='0']").attr('checked', true);
		} else if (${pvo.room} =="1"){
			$(":radio[name='room'][value='1']").attr('checked', true);
		} else if (${pvo.room} =="2"){
			$(":radio[name='room'][value='2']").attr('checked', true);
		}
		if(${pvo.shower} =="0"){
			$(":radio[name='shower'][value='0']").attr('checked', true);
		} else if (${pvo.shower} =="1"){
			$(":radio[name='shower'][value='1']").attr('checked', true);
		} else if (${pvo.shower} =="2"){
			$(":radio[name='shower'][value='2']").attr('checked', true);
		}
		if(${pvo.fulloption} =="0"){
			$(":radio[name='fulloption'][value='0']").attr('checked', true);
		} else if (${pvo.fulloption} =="1"){
			$(":radio[name='fulloption'][value='1']").attr('checked', true);
		}
